    ...
    <ol className={`flex clex-col md:flex-row relative`}>
        <li className='mb-5 ml-4'>
            <p className='flex flex-wrap gap-3 flex-row items-center justify-start text-xs md:text-sm'>
                <span className='inline-block px-2 py-1 text-white dark:text-black bg-[#303030] dark:bg-gray-200 rounded-md hover:scale-105 transition-transform duration-300 cursor-pointer'>
                    {typeof date === 'string' ? date : date.toLocaleDateString('ko-KR', { year: 'numeric', month: '2-digit', day: '2-digit' })}
                </span>
                <span className='text-xl text-black dark:text-gray-300 pt-[2px]'>{title}</span>
